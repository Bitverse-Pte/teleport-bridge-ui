...
/* export function useTokenBalancesWithLoadingIndicator(address?: string, tokens?: (Token | undefined)[]): [{ [tokenAddress: string]: CurrencyAmount<Token> | undefined }, boolean] {
  ...
  const anyLoading: boolean = useMemo(() => balances.some((callState: { loading: any }) => callState.loading), [balances])
...
export function useAggregateUniBalance(): CurrencyAmount<Token> | undefined {
  ...
  return CurrencyAmount.fromRawAmount(uni, JSBI.add(JSBI.add(uniBalance?.quotient ?? JSBI.BigInt(0), uniUnclaimed?.quotient ?? JSBI.BigInt(0)), uniUnHarvested?.quotient ?? JSBI.BigInt(0)))
}
 */
